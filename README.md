...
**基于 macode.cloud / OpenAI 兼容图像 API 的图片生成与编辑工具**

提供简洁精美的 Web UI，默认支持 macode.cloud 兼容接口、macode.cloud 队列通道与可导入的自定义 HTTP 服务商。<br>
...
- **多服务商接入**：内置 macode.cloud 兼容接口（含 `Images API` 和 `Responses API`）、macode.cloud 队列通道，并支持通过 JSON 导入自定义 HTTP 服务商配置（兼容同步/异步任务）。
- **API 代理**：macode.cloud 兼容接口与队列通道均可配置自定义代理。其中兼容接口可开启同源 `/api-proxy/` 代理，交由 Docker 或本地开发环境转发至真实 API，绕开浏览器 CORS 限制。
...
**配置默认 API URL**：在 Vercel 项目的 **Settings → Environment Variables** 中添加 `VITE_DEFAULT_API_URL`（如 `https://www.macode.cloud/v1`），然后重新部署即可生效。
...
```bash
VITE_DEFAULT_API_URL=https://www.macode.cloud/v1 npm run deploy:cf
```

PowerShell 示例：

```powershell
$env:VITE_DEFAULT_API_URL="https://www.macode.cloud/v1"; npm run deploy:cf
```
...
- `DEFAULT_API_URL`：设置页面上默认显示的 API 地址（如 `https://www.macode.cloud/v1`）。也支持填写 `.json` 配置 URL 或带 `settings` 参数的分享 URL 来导入自定义服务商配置（详见下方说明）。
- `API_PROXY_URL`：配置内置代理实际转发到的完整 API 基础地址（仅开启代理时有效）。代理不会自动补 `/v1`，macode.cloud 兼容接口通常必须填写到版本前缀，如 `https://www.macode.cloud/v1`。
...
>
> - **macode.cloud 兼容接口**：将 `DEFAULT_API_URL` 留空或填写一个占位地址（如 `https://proxy`）。
...
docker run -d -p 8080:80 \
  -e DEFAULT_API_URL=https://www.macode.cloud/v1 \
  -e ENABLE_API_PROXY=true \
  -e LOCK_API_PROXY=true \
  -e API_PROXY_URL=https://www.macode.cloud/v1 \
  ghcr.io/macode/gpt_image_playground:latest
```

**隐藏真实 API 地址示例（macode.cloud 兼容接口）：**
...
services:
  gpt-image-playground:
    ...
    environment:
      - DEFAULT_API_URL=https://www.macode.cloud/v1
    ports:
      - "8080:80"
...
你可以在项目根目录新建 `.env.local` 文件配置默认 API URL（如 `VITE_DEFAULT_API_URL=https://www.macode.cloud/v1`）。然后安装依赖并启动：
...
修改 `dev-proxy.config.json`，将 `target` 设置为真实的完整 API 基础地址。代理不会自动补 `/v1`，macode.cloud 兼容接口通常必须填写到版本前缀，如 `https://api.example.com/v1`。重启开发服务器后，在页面设置中开启 **API 代理** 即可（请求将被转发如 `http://localhost:5173/api-proxy/... -> target/...`）。此功能仅在 `npm run dev` 阶段生效，不会影响打包产物。

...
**配置默认 API URL**：在 Vercel 项目的 **Settings → Environment Variables** 中添加 `VITE_DEFAULT_API_URL`（如 `https://macode.cloud/v1`），然后重新部署即可生效。
...
```bash
VITE_DEFAULT_API_URL=https://macode.cloud/v1 npm run deploy:cf
```

PowerShell 示例：

```powershell
$env:VITE_DEFAULT_API_URL="https://macode.cloud/v1"; npm run deploy:cf
```
...
- `DEFAULT_API_URL`：设置页面上默认显示的 API 地址（如 `https://macode.cloud/v1`）。也支持填写 `.json` 配置 URL 或带 `settings` 参数的分享 URL 来导入自定义服务商配置（详见下方说明）。
- `API_PROXY_URL`：配置内置代理实际转发到的完整 API 基础地址（仅开启代理时有效）。代理不会自动补 `/v1`，macode.cloud 兼容接口通常必须填写到版本前缀，如 `https://macode.cloud/v1`。
...
docker run -d -p 8080:80 \
  -e DEFAULT_API_URL=https://macode.cloud/v1 \
  -e ENABLE_API_PROXY=true \
  -e LOCK_API_PROXY=true \
  -e API_PROXY_URL=https://macode.cloud/v1 \
...
services:
  gpt-image-playground:
    ...
    environment:
      - DEFAULT_API_URL=https://macode.cloud/v1
    ports:
      - "8080:80"
...
你可以在项目根目录新建 `.env.local` 文件配置默认 API URL（如 `VITE_DEFAULT_API_URL=https://macode.cloud/v1`）。然后安装依赖并启动：
